...
interface IBasket {
  ...
  function withdrawAbleProfit(address investor) external view returns (uint256 amount);
  
  function deposit(uint256 amount, bytes32 r,bytes32 s) external returns(bool success);
